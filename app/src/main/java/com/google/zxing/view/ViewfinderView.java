...
public final class ViewfinderView extends View {
  ...
  public ViewfinderView(Context context, AttributeSet attrs) {
    ...
    labelTextColor = array.getColor(R.styleable.ViewfinderView_label_text_color, 0xFFFFFF);
  ...
  private void drawTextInfo(Canvas canvas, Rect frame) {
    ...
    //调整文本位于扫描框的下面  第三个参数(调整文本框位于扫描框的上面-frame.bottom，文本框位于扫描框的上面-frame.top)
    //调整文本与扫描框的距离  第三个参数（后面加上距离）
    canvas.drawText(labelText, frame.left + frame.width() / 2, frame.bottom + CORNER_RECT_HEIGHT+30, paint);
  }
